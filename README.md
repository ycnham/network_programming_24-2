...
1. AccessGame.java
   ->
   StartingUI.java로 수정함, UI는 변함 없음 (게임 초기 접속 화면)

2. client/ChatClientMain.java에 채팅 ui 관련 코드까지 모두 포함되어 있어서 실행 시 게임 로직뿐만 아니라 게임 화면 출력까지 한꺼번에 동작하던 부분
   ->
   채팅 ui 관련 코드는 ui/GameUI.java에 분리함
   ui/ConnectionPanel.java에서 GameUI.java를 호출해서 게임 화면을 띄우는 방식으로 로직 변경함
